...
#define CAN_ID_SNS_DHT                 (0x100U) // 온습도 수신 (v2.0)
#define CAN_ID_SNS_LUX                 (0x110U) // 조도 수신 (v2.0)
#define CAN_ID_ACT_FEEDBACK            (0x130U) // 액추에이터 실행 결과 수신
#define CAN_ID_CMD_WIPER               (0x200U) // 와이퍼 제어 명령 (v2.0)
#define CAN_ID_CMD_LIGHT               (0x210U) // 조명 제어 명령 (v2.0)
#define CAN_ID_CMD_TURN                (0x220U) // 방향지시등 제어 명령 (v2.0)
#define CAN_ID_CON_MODE_SET            (0x300U) // 콘솔로부터 모드 설정 수신 (v2.0)
#define CAN_ID_SNS_ALIVE               (0x2F0U) // 센서 생존 확인

/* ===== 노드 식별자 (Sender ID) ===== */
#define SENDER_ID_GATEWAY              (1U)
#define SENDER_ID_SENSOR               (2U)
#define SENDER_ID_ACTUATOR             (3U)
#define SENDER_ID_CONSOLE              (4U)

/* ===== 제어 대상 코드 ===== */
#define TARGET_HEADLIGHT               (0x01U)
#define TARGET_TAILLIGHT               (0x02U)
#define TARGET_WIPER                   (0x03U)
#define TARGET_TURN                    (0x04U)
...
// [v2.0] 시스템 운영 모드 (Console -> Gateway 동기화용)
typedef struct
{
    uint8_t wiper_mode;   // 0:AUTO, 1:OFF, 2:LOW, 3:HIGH
    uint8_t light_mode;   // 0:AUTO, 1:OFF, 2:ON
    uint8_t turn_mode;    // 0:OFF, 1:LEFT, 2:RIGHT, 3:HAZARD
} SystemMode_t;
...
static inline uint8_t protocol_calculate_crc8(const uint8_t *data, uint8_t len)
{
    ...
    for (uint8_t i = 0; i < len; i++)
    {
        ...
        {
            if (crc & 0x80) crc = (crc << 1) ^ 0x1D; // SAE J1850 Polynomial
            else crc <<= 1;
        }
    }
    return crc;
...
static inline int protocol_parse_env_v22(const uint8_t *data, uint8_t dlc, float *h, float *t)
{
    ...
    // 2. 출처 검사: B0에 적힌 Sender ID가 '센서 노드(2)'가 맞는가?
    if (data[0] != SENDER_ID_SENSOR) return -2; // v2.0: Sensor ID check
    
    // 3. 값 추출 (v2.0 Little-Endian): 
    // B2-3: Humidity (x10), B4-5: Temperature (x10)
    uint16_t h_raw = (uint16_t)((data[3] << 8) | data[2]);
    uint16_t t_raw = (uint16_t)((data[5] << 8) | data[4]);
...
static inline int protocol_parse_lux_v22(const uint8_t *data, uint8_t dlc, uint16_t *lux)
{
    ...
    if (data[0] != SENDER_ID_SENSOR) return -2;
    
    // 조도 값은 2바이트(B2~3)에 Little-Endian 방식으로 담겨 있어.
    *lux = (uint16_t)((data[3] << 8) | data[2]);
    return 1;
}
