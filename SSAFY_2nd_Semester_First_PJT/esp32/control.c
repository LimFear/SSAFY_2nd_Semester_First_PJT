...
#define WIPER_CMD_SET_SPEED_LEVEL   0x01
...
static void canPollCommands()
{
    for (;;)
    {
        ...
        if (rx_sender != SENDER_ID_GATEWAY) {
        ...
        if (rxMessage.identifier == CAN_ID_CMD_WIPER)
        {
            if (rxMessage.data_length_code < 4) { // v2.0 기준으로 B2, B3 사용하므로 최소 4바이트
                continue;
            }

            /* v2.0: B0:sender, B1:rolling, B2:command, B3:value */
            uint8_t command = rxMessage.data[2];
            uint8_t speedLevel = rxMessage.data[3];

            if (command != WIPER_CMD_SET_SPEED_LEVEL) {
                continue;
            }
        ...
        if (rxMessage.identifier == CAN_ID_CMD_LIGHT)
        {
            if (rxMessage.data_length_code < 4) {
                continue;
            }

            /* v2.0: B2:command, B3:state */
            uint8_t command = rxMessage.data[2];
            uint8_t highState = rxMessage.data[3];

            if (command != LIGHT_CMD_SET_STATE) {
                continue;
            }
        ...
        if (rxMessage.identifier == CAN_ID_CMD_TURN)
        {
            ...
            /* v2.0: B2:command, B3:value */
            uint8_t command = rxMessage.data[2];
            uint8_t value = rxMessage.data[3];
